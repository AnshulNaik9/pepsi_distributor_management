...
import { Receipt, Package, Banknote, LogOut, Navigation } from "lucide-react";

export function DriverLayout({ children }: { children: ReactNode }) {
  const [location] = useLocation();

  const navItems = [
    { label: "Billing", url: "/driver/billing", icon: Receipt },
    { label: "Stock", url: "/driver/stock", icon: Package },
  ...
  return (
    <div className="bg-slate-50 min-h-screen flex flex-col mx-auto max-w-md shadow-2xl relative">
      {/* Driver Header */}
      <header className="bg-primary px-4 py-4 rounded-b-2xl shadow-lg shadow-primary/20 sticky top-0 z-20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-primary-foreground">
            <Navigation className="w-6 h-6" />
            <h1 className="font-display font-bold text-lg">Delivery Mode</h1>
          </div>
          <Link href="/">
